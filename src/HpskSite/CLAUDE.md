...
- **Known, accepted:** a shooter with partial station results but no finishing time appears in the standings as an unranked `rank 0 / Totaltid -` row (pre-existing `GetSpringskytteResults` behaviour, not introduced here).
