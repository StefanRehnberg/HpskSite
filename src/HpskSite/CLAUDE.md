...
**Springskytte: "Nu på banan" band + paged standings (2026-08-09).** Built for a wall TV at SM, where one weapon class is on screen at a time (C + stafett Saturday, A Sunday) and the field is 100+ — so `?tiles=`/`?vaxla=` are the wrong tools here; the asks were *fit more shooters* and *show who is running*.
- **`Springskytte/GetSpringskytteBoardState(competitionId, weaponClass?)`** (new, anonymous like the board itself) diffs the **published** start list against entered results: a starter whose time has passed with no result row is **on course**; later ones are **upcoming**. `GetSpringskytteResults` cannot answer this — it only ever returns shooters who already have a result row, so the in-progress field was invisible.
- **"Now" is resolved server-side** and echoed as `serverNowSec`; the client ticks the elapsed clocks locally from that baseline. The board machine is a borrowed kiosk PC and a skewed clock would otherwise empty the band or strand finished shooters in it.
- **Self-cleaning:** an on-course entry older than **45 min** is dropped (a lap is minutes, so that's a missing result, not a runner). Band caps at 12 rows and shows `(shown/total)` when truncated. Dated lists for another day are skipped (`ListDate` vs today) — start times are time-of-day only, so Sunday's A list would otherwise read as live mid-Saturday.
...
- Verified 31/31 via `C:\Repos\hpsk-verify\liveboard-springskytte-verify.mjs` (dev comp 6628). The band is time-relative, so the test data needs re-timing before a run — see the seeder note in that script's header.
