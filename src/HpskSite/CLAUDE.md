...
### Särskjutning (Shoot-Off) for Championship Medal Positions ✅ (2026-05-19, gate corrected 2026-08-16, grouping corrected 2026-09-07)
**Rule:** In Championship competitions (`competitionScope` ∈ {`Svenskt Mästerskap`, `Landsdelsmästerskap`, `Kretsmästerskap`, `Klubbmästerskap`}), tied medal positions 1–3 are resolved by a 5-shot shoot-off, repeated until separated. **The normal tie-breakers do not decide a medal position** — not X-count, not series countback. Ranks 4+ continue to use X-count + countback as before.

⚠️ *"None of the normal tie-breakers apply"* used to be written here without qualification, and that overstates it: `ApplyShootOffOverride` only re-orders **when shoot-off entries exist** (`maxRound > 0`). Until then the base sort stands, so the list shows a **provisional** X-count order rather than locking. Verified 2026-09-07 on SSM 2026: a 476 tie displays 476/14X ahead of 476/10X while the shoot-off is still pending.

⚠️ **SHB `F.6.11.2.1.1` distinguishes the levels:** shoot-off *"skall"* happen at SM, landsdels- **or kretsmästerskap**, but only *"bör"* at föreningsmästerskap. The code gates on all four via `IsChampionshipScope`. Checked 2026-09-07 and left as is — since nothing is re-ordered or blocked without entries, surfacing the card at a klubbmästerskap **is** the "bör". Don't "fix" it into a hard four-level requirement.

**⚠️ WHICH GROUP, and among WHOM (corrected 2026-09-07).** Detection used to walk the **result list's class groups** — i.e. the skill classes — and let any shooter through who had finished their own series. Both are wrong: a medal is decided per **mästerskapsklass (finalklass)** and only among those who can win it.
- Groups come from `CompetitionTypes/Common/ChampionshipCategory` — A, B, C plus Dam C / Vet Y C / Vet Ä C / Jun C. **Classes 1–3 are competence levels, not championship classes.**
- Only **finalists** are contenders when the competition has a finals round; finalists unknown = nobody, same caution as the gate below.
- Ties land on `PrecisionFinalResults.MedalCategoryTies` (competition level), **not** on `classGroup.TiedMedalGroups`, because the result list's grouping is a different partition from the medal's. `GetShootOffStatus` keeps the identical JSON shape (`className` carries the category name) so the admin card needed no change.
- Without a finals round there is no finalklass and the rule says nothing → old per-class behaviour retained deliberately.
- Measured on SSM 2026: **five** wrongly-flagged groups (C2/B2/A1 = cut shooters; C3/C3 Dam = finalists in the wrong group) collapsed to **one** correct one. Note C3 showed "Silver + Brons" while the real tie in category C is for **bronze** — a C1 shooter sits second overall. Wrong grouping gives the wrong *medal*, not just the wrong label.
- ⚠️ **Fältskytte still groups per class group** in its own `FaltskytteController.GetFaltskytteResults` path. The finalists half doesn't apply there (no finals), but the category half plausibly does. Not investigated.
...
- `CalculateFinalResults` runs the existing sort, then on championship comps builds the **championship-category standings from the finalists**, detects tied medal tiers there, and re-orders that slice using shoot-off entries. Emits `finalResults.medalCategoryTies[]` (admin payload, keyed by category). `classGroup.tiedMedalGroups[]` is only used on the no-finals-round path. Resolved-tie footnotes go on the category **and** are repeated on each `classGroup.shootOffNotes[]` whose shooters are involved — the public page renders footnotes per class table, so a category-only note would be invisible.
