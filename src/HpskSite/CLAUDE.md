...
**Flow (revised — TWO steps, no manual yearly sign-off):** (1) shooter submits a Guldserie/Snabbserie; (2) a functionary validates each series (queue or QR). The year's Guldfodring **auto-completes** when Part 1 (3 validated qualifying Guld series) + Part 2 (validated Guld Snabbserie OR fält-medal) are both met — `MarkenController.RecomputeYearlyQualificationAsync` runs after every series Verify/Reject and lazily on read, upserting/downgrading the `MemberBadgeQualification` row (which the årtalsmärke ladder counts). There is **no** "Signera guldfodring" button (removed `SignOffGuldfodring`/`SignOffQualification`/`RejectQualification`).
