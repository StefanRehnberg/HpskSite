...
**Patrullista page** — routed `Controllers/FaltskyttePatrolListController.cs` (`/patrullista/{competitionId}`) → `Views/FaltskyttePatrolList.cshtml`. The competition page's "Visa patrullista" now opens this **public, chromeless, auto-reloading (60 s)** page (clubhouse screen) instead of a modal — gated server-side on `faltskyttePatrolsPublished`, **no login** (intentional visibility change from the old login-gated modal; a screen isn't logged in).
