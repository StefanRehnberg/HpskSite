    ...
    <p style='color: #999; font-size: 12px;'>
            ...
            string tutorialId)
        {
            ...
            if (!string.IsNullOrEmpty(tutorialId))
            {
                ...
                // Match the existing email pattern (see Marketing/hallandsserien-invitation-email.html):
                // link to the /tutorials hub page — its filter buttons (Medlem / Klubbadmin / Admin)
                // get the recipient to the right guide. The ?tutorial= deep-link parameter
                // exists in tutorialHelper.js but has been unreliable from email clients,
                // so we keep it simple here.
                var tutorialUrl = $"{siteUrl}/tutorials";
